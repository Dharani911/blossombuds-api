...
export default function RegisterModal() {
  ...
  useEffect(() => { sheetRef.current?.querySelector<HTMLInputElement>("input")?.focus(); }, []);

  const emailOk = /^\S+@\S+\.\S+$/.test(email);
  const nameOk = name.trim().length >= 2;
  const pwOk = password.length >= 8;
  const canSubmit = emailOk && nameOk && pwOk;
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { token } = await customerRegister({ name, email, password });
  ...
  const modal = (
    <>
      ...
      <div className="auth-modal" role="dialog" aria-modal="true" aria-label="Register form">
        <div ref={sheetRef} className="sheet sheet-in">
          ...
          <form className="form" onSubmit={onSubmit}>
            ...
            <label className="label">
              <span>Password</span>
              <div className="field">
                <input className="input" type={showPw ? "text" : "password"} placeholder="At least 8 characters" value={password} onChange={(e)=>setPassword(e.target.value)} required />
                <button
                  type="button"
...
/* Secondary as plain text link */
.links{
  display:flex; justify-content: center; gap: 6px; align-items:center; margin-top: 8px;
}
.links.single{ justify-content: flex-end; gap: 8px; }
