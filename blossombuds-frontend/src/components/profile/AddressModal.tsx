...
import { getAllStates, getDistrictsByState, getCountries, type State, type District, type Country } from "../../api/geo";
...
export default function AddressModal({
  ...
  const [stateId, setStateId] = useState<number | "">(
    typeof initial?.stateId === "number"
      ? initial!.stateId
      : (initial?.stateId ? Number(initial!.stateId) : "")
  );
  const [districtId, setDistrictId] = useState<number | "">(
    typeof initial?.districtId === "number"
      ? initial!.districtId
      : (initial?.districtId ? Number(initial!.districtId) : "")
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!alive) return;
        setDistricts(ds || []);
        // When editing: if initial had a districtId, keep it if it still exists in this state's list
        const initDid =
          typeof initial?.districtId === "number"
            ? initial!.districtId
            : (initial?.districtId ? Number(initial!.districtId) : undefined);
        if (initDid && ds?.some(d => d.id === initDid)) {
          setDistrictId(prev => (prev ? prev : initDid));
        } else {
          // if current districtId does not belong to this state, clear it
        ...
        <div className="modal-body">
          ...
          <div className="grid">
            <Field label="Full name">
              <input className="input" value={name} onChange={e=>setName(e.target.value)} placeholder="Receiver's full name" />
            </Field>
            <Field label="Phone">
            ...
            <Field label="Address line 1">
              <input className="input" value={line1} onChange={e=>setLine1(e.target.value)} placeholder="House / Flat / Street" />
            </Field>
            ...
            <Field label="State">
              ...
              >
                <option value="">{loadingLoc && states.length===0 ? "Loading…" : "Select state…"}</option>
            ...
            <Field label="District">
              ...
              >
                <option value="">{!stateId ? "Select state first…" : (loadingLoc ? "Loading…" : "Select district…")}</option>
                {districts.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
              </select>
            </Field>

            <Field label="Pincode">
              <input className="input" value={pincode} onChange={e=>setPincode(e.target.value)} placeholder="e.g., 626161" />
            </Field>
...
.check{ display:flex; align-items:center; gap:8px; font-weight:800; margin-top:2px; }
