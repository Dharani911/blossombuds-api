...
.items{ padding:8px; }
.row{ display:grid; grid-template-columns: 96px 1fr 140px 110px 36px; gap:10px; align-items:center; padding:10px 8px; border-bottom:1px dashed var(--ink); }
...
.thumb .ph{
  ...
  background: radial-gradient(120px 60px at -20px 50%, #ffe9a8, #ffd3e1 60%, #fff);
...
.line{ font-weight:900; color: var(--accent); text-align:right; }

/* ===== Fancy Quantity Stepper ===== */
.qty{ display:inline-flex; align-items:center; justify-self:center; gap:8px; background:#fff;
  padding:4px; border:1px solid var(--ink); border-radius:12px; box-shadow:0 6px 18px rgba(0,0,0,.06); }
.btn-icon{
  width:30px; height:30px; border-radius:10px; border:1px solid var(--ink);
  background: linear-gradient(180deg, #fff, #fafafa);
  cursor:pointer; display:grid; place-items:center; font-size:18px; line-height:0;
  transition: transform .1s ease, box-shadow .1s ease, border-color .1s ease, background .1s ease;
}
.btn-icon:hover{ transform: translateY(-1px); box-shadow:0 8px 18px rgba(0,0,0,.10); background:#fff; border-color: rgba(0,0,0,.12); }
.btn-icon:active{ transform: translateY(0) scale(.98); }

.qty input{
  width:64px; height:32px; border:1px solid var(--ink); border-radius:10px; text-align:center; font-weight:800;
  outline:none; background:#fff; transition: box-shadow .12s ease, border-color .12s ease;
}
.qty input:focus{ border-color: var(--gold); box-shadow: 0 0 0 3px rgba(246,195,32,.18); }

.rm{ background:transparent; border:none; cursor:pointer; width:32px; height:32px; border-radius:10px; display:grid; place-items:center; }
          ...
          <section className="card items">
            ...
            {items.map(it=>(
              <div className="row" key={it.id}>
                {/* Prefer productId saved in the cart item; else parse from "productId:variant" */}
                {/* @ts-ignore */}
                <Thumb
                  productId={(it.productId as number) || Number(String(it.id).split(":")[0])}
                  src={it.image}
                  alt={it.name}
                />
                ...
                <div className="qty" aria-label="Quantity chooser">
                  <button className="btn-icon" onClick={()=>setQty(it.id, Math.max(1, it.qty-1))} aria-label="Decrease">−</button>
                  <input
                    type="number"
                    min={1}
                    value={it.qty}
                  ...
                  />
                  <button className="btn-icon" onClick={()=>setQty(it.id, it.qty+1)} aria-label="Increase">+</button>
                </div>

                <div className="line" aria-label="Line total">{inr(it.price*it.qty)}</div>

                <button className="rm" onClick={()=>remove(it.id)} aria-label="Remove">✕</button>
              </div>
            ))}
          </section>
